...
async fn recover_only_terminal_orders_no_sync() {
    let dir = tempfile::tempdir().unwrap();
    let oms = build_oms_with_csv(dir.path());
    let mut order = oms.create_order(&make_input("CLI-R1"), Local::now()).unwrap();
    ...
    oms.cancel_order(&mut order, "测试", Local::now()).await.unwrap();
...
async fn recover_persists_across_instances() {
    ...
    {
        let oms = build_oms_with_csv(dir.path());
        oms.create_order(&make_input("CLI-PERSIST-1"), Local::now()).unwrap();
    }
    // 重新创建实例（模拟重启）
...
async fn recover_with_active_orders_via_csv() {
    let dir = tempfile::tempdir().unwrap();
    {
        ...
        for i in 0..3 {
            oms.create_order(
                &make_input(&format!("CLI-ACTIVE-{}", i)),
                Local::now(),
            )
            .unwrap();
        }
...
async fn sync_order_without_exchange_id_noop() {
    let dir = tempfile::tempdir().unwrap();
    let oms = build_oms_with_csv(dir.path());
    let mut order = oms.create_order(&make_input("CLI-SYNC-1"), Local::now()).unwrap();
...
async fn recovery_report_summary_chinese() {
    let dir = tempfile::tempdir().unwrap();
    let oms = build_oms_with_csv(dir.path());
    oms.create_order(&make_input("CLI-REC"), Local::now()).unwrap();
    let report = oms.recover().await;
